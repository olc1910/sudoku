...
class Sudoku:
    ...
    def toSVG(self, empty_as_zero=True):
        # Variables
        cell_size = 40
        line_color = "black"
            ...
            for column in range(9):
                if self.board[row][column] != 0:
                    if empty_as_zero:
                        ...
                        svg += f'<text x="{(column + 0.5) * cell_size}" y="{(row + 0.5) * cell_size}" \
                                    style="font-size:20; text-anchor:middle; dominant-baseline:middle"> {"." if self.board[row][column] == 0 else str(self.board[row][column])} </text>'
    ...
    now = datetime.now()
    filename_dot = f'sudoku-{now:%Y}_{now:%m}_{now:%d}_{now:%H}_{now:%M}_dot.txt'
    filename_zero = f'sudoku-{now:%Y}_{now:%m}_{now:%d}_{now:%H}_{now:%M}_zero.txt'

    def write_dot(self):
        with open(Sudoku.filename_dot, 'w') as file:
            for i in range(9):
                row_string = " ".join([str(x) if x != 0 else "." for x in self.board[i]])
                file.write(row_string + '\n')

    def write_zero(self):
        with open(Sudoku.filename_dot, 'r') as dot:
                game_dot = dot.read()
                game_dot = game_dot.replace('.', '0')
        with open(Sudoku.filename_zero, "w") as zero:
            zero.write(game_dot)
...
def main():
    ...
    # creating the .svg-File with current date, time, and difficulty
    svg = sudoku.toSVG(empty_as_zero=True)  # Change this argument to False if you want dots as empty cells
    now = datetime.now()
    name = f'sudoku-{now:%Y}_{now:%m}_{now:%d}_{now:%H}_{now:%M}_{now:%S}-{difficulty}.svg'
